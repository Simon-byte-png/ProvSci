...
当前仓库已经从“架构 Demo”进入 v0.3 工程原型。下一步的重点不是继续堆功能，而是把真实文献输入、证据定位、结构化结果、验证、人工复核和固定评测做得足够可靠，并形成一次可复现的端到端演示。

你的 ProvSci 任务与会议中另外两条线的边界如下：

- **数据智能体**：继续负责结果数据挖掘与处理原型，补齐真实文献、来源与证据、结构化字段、验证、失败样本和 Benchmark，最终形成可交付的数据集/数据库原型，并据实验结果判断走方法论文、数据库论文还是产品验证。
- **长程状态维护算法**：核心图结构、规范图/行为图/观测图和算法方案由学姐推进；方案明确后，你配合代码复现、Benchmark、对比和消融实验，不需要把它混进 ProvSci 第一版。
- **综述**：继续修改、补图并选择合适期刊投稿；它是单独的论文任务，不是数据智能体的验收条件。
- **钙钛矿和公司生产设备案例**：属于另一条应用线，不是你当前 ProvSci 的默认领域或个人任务。

具体要求、任务边界和最终验收标准见：[ProvSci 下一阶段要求](docs/ProvSci_下一阶段要求.md)。
